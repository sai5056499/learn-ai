...
import { LearningItem, ContentBlock, Flashcard, Resource, PracticeProblem, Progress } from '../../types';
...
import { ChevronRightIcon, TextIcon, CodeIcon, QuizIcon, DownloadIcon, LoadingSpinnerIcon, FlashcardsIcon, VideoIcon, DocumentIcon, ArticleIcon, CheckIcon, LeetCodeIcon, GfgIcon, ClockIcon, DiagramIcon, InteractiveModelIcon, HyperparameterIcon, BugAntIcon, LightBulbIcon, ChatBubbleOvalLeftEllipsisIcon } from '../common/Icons';

interface LessonAccordionProps {
    courseId: string;
    item: LearningItem & { type: 'lecture' };
    progress: Progress;
...
const LessonAccordion: React.FC<LessonAccordionProps> = ({ courseId, item, progress, onViewFlashcards }) => {
    const { 
        flashcardModalState,
        storyModalState,
    ...
    const completionTimestamp = progress.get(item.id);
    const needsReview = completionTimestamp ? isDueForReview(completionTimestamp) : false;
    const isGeneratingFlashcards = flashcardModalState.isLoading && flashcardModalState.title === item.title;
    const isGeneratingStory = storyModalState.isLoading && storyModalState.title === item.title;
    ...
    const handleSaveNoteClick = () => {
        handleSaveItemNote(courseId, item.id, note);
        setIsNoteSaved(true);
    ...
    const handleCheckboxClick = (e: React.MouseEvent) => {
        e.stopPropagation();
        handleToggleItemComplete(courseId, item.id);
    };

    const handleFlashcardClick = () => {
        handleShowTopicFlashcards(item);
    };
    
    const handleStoryClick = (e: React.MouseEvent) => {
        e.stopPropagation();
        handleShowTopicStory(item);
    };
        ...
        return (
            <div key={block.id} className="flex items-start space-x-4 p-4 bg-white rounded-md relative group border border-gray-200">
                ...
                <div className="flex-grow">
                    ...
                    {block.type === 'diagram' && block.diagram && <DiagramBlock mermaidString={block.diagram} blockId={block.id} onUpdateDiagram={(newSyntax) => handleUpdateContentBlock(courseId, item.id, block.id, newSyntax)} />}
    ...
    return (
        <div className={`bg-gray-50 rounded-lg transition-opacity ${isComplete ? 'opacity-60' : 'opacity-100'}`}>
            <div className="flex justify-between items-center p-2 text-left hover:bg-gray-100 transition-colors rounded-lg">
                ...
                <button onClick={() => setIsOpen(!isOpen)} className="flex-grow flex items-center gap-2 text-left mx-2" aria-expanded={isOpen} aria-controls={`lesson-content-${item.id}`}>
                    ...
                    {needsReview && <span title="Due for review" className="flex-shrink-0"><ClockIcon className="h-4 w-4 text-yellow-500"/></span>}
                     <div className="flex-grow relative">
                        <h3 className={`font-semibold text-slate-700 transition-colors ${isComplete ? 'line-through text-gray-400' : ''}`}>{item.title}</h3>
                        <p className="text-sm text-gray-500 hidden sm:block truncate">{item.data.objective}</p>
                        {showAnalogy && <AnalogyPopover concept={item.title} onClose={() => setShowAnalogy(false)} />}
                    </div>
                </button>
                <div className="flex items-center flex-shrink-0">
                    <button onClick={handleStoryClick} disabled={isGeneratingStory} className="ml-2 p-1.5 text-gray-500 rounded-full hover:bg-blue-100 hover:text-blue-600 transition-colors disabled:opacity-50 disabled:cursor-wait" title="Story Mode">
                        {isGeneratingStory ? <LoadingSpinnerIcon className="h-5 w-5" /> : <ChatBubbleOvalLeftEllipsisIcon className="h-5 w-5" />}
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); setShowAnalogy(!showAnalogy); }} className="ml-1 p-1.5 text-gray-500 rounded-full hover:bg-yellow-100 hover:text-yellow-600 transition-colors" title="Get Analogy">
                        <LightBulbIcon className="h-5 w-5" />
                    </button>
                    <button onClick={handleFlashcardClick} disabled={isGeneratingFlashcards} className="ml-1 p-1.5 text-gray-500 rounded-full hover:bg-gray-200 hover:text-green-600 disabled:opacity-50 disabled:cursor-wait transition-colors" title={isGeneratingFlashcards ? "Generating..." : "Generate Flashcards"}>
                        {isGeneratingFlashcards ? <LoadingSpinnerIcon className="h-5 w-5" /> : <FlashcardsIcon className="h-5 w-5" />}
                    </button>
            ...
            <div id={`lesson-content-${item.id}`} className={`grid overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                <div className="min-h-0">
                    <div className="p-3 border-t border-gray-200 bg-white">
                        <div ref={contentRef} className="space-y-4">
                            ...
                            {item.data.resources && item.data.resources.length > 0 && (
                                <div className="p-4 bg-gray-50 rounded-md border border-gray-200">
                            ...
                            {item.data.practiceProblems && item.data.practiceProblems.length > 0 && (
                                <div className="p-4 bg-gray-50 rounded-md border border-gray-200">
                                    <h4 className="font-semibold text-slate-700 mb-2">Practice Problems</h4>
                                    <ul className="space-y-2">
                                        {item.data.practiceProblems.map((problem, index) => (
                                            <li key={index} className="flex items-center">
